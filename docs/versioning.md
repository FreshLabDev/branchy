...
Branchy is past `v1.0.0` and on the `v1.2.x` line.
...
v1.2.1-alpha.N  current pre-release line
```

Pre-releases up to `v1.2.1-alpha.2` were tagged on `main`, under the earlier
rule that put every tag there. From now on they are tagged on `dev`.

Branchy has no standing test bot. A change to webhook handling, the outbox, or
the OAuth flow is worth asking for one, because all three fail in ways that are
invisible until a real GitHub delivery arrives.
