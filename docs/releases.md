...
```sh
docker pull ghcr.io/freshlabdev/branchy:<tag>
docker inspect --format '{{index .RepoDigests 0}}' ghcr.io/freshlabdev/branchy:<tag>
```

That first pull is not a formality. Deploying straight to
`ghcr.io/freshlabdev/branchy@sha256:<digest>` on a host that has never fetched
